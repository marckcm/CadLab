...
def gerar_senha(request, exame_id):
    ...
    if exame.senha:
        return FileResponse(gerar_pdf_exames(
            exame.exame.nome, exame.usuario.first_name, exame.senha
            ), filename='token.pdf')
...
def alterar_dados_exame(request, exame_id):
    ...
    if requer_senha and (not exame.senha):
        ...
        return redirect('/empresarial/exame_cliente/{exame_id}')
    ...
    return redirect(f'/empresarial/cliente/{exame_id}')

...
def gerar_senha_aleatoria(tamanho):
    ...
    for i in range(0, qtd):
        caracteres += choice(caracteres_especiais)

    senha = list(letras + numeros + caracteres)
...
def gerar_pdf_exames(exame, paciente, senha):
    path_template = os.path.join(settings.BASE_DIR, 'templates/parciais/_senha_exame.html')

...
from .models import TiposExame, PedidosExames, SolicitacaoExame, AcessoMedico
...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        messages.add_message(request, constants.ERROR,'Pedido não pode ser cancelado.')
        return redirect('/exames/gerenciar_pedidos/')
...
def acesso_medico(request, token):
    ...
    if acesso_medico.status == 'Expirado':
        messages.add_message(request, constants.ERROR, 'Esse token está espirado, favor solicitar outro')
